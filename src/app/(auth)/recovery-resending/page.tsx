...
const ResendLinkPage = () => {
  ...
  return (
    <div className="flex items-center justify-center px-4 rounded-[8px] flex-col w-[378px] p-6">
      <p className="text-h1 text-center mb-5">Email verification link expired</p>
      <Modal open={modal.open} onClose={closeModal} modalTitle={modal.title}></Modal>
      <p className="text-xs ">
        Looks like the verification link has expired. <br />
        Not to worry, we can send the link again.
      </p>
      <div className=" mt-6 mb-6">
        <Button type={'submit'} variant={'primary'} onClick={handleResend} disabled={!email} width={'300px'}>
          Resend Link
        </Button>
      </div>
